...
async function main() {
  ...
  const [{ regclass }] =
    await sql`select to_regclass('public.examples') as regclass;`;
  console.log("public.examples exists as:", regclass);

  const columns =
    await sql`select column_name, data_type, is_nullable, column_default from information_schema.columns where table_schema = 'public' and table_name = 'examples' order by ordinal_position;`;
  console.log("examples columns:", columns);

  const [{ regclass: regChallenges }] =
    await sql`select to_regclass('public.challenges') as regclass;`;
  console.log("public.challenges exists as:", regChallenges);

  const [{ regclass: regSubmissions }] =
    await sql`select to_regclass('public.submissions') as regclass;`;
